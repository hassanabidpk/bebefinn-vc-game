...
const APPROVED_PROMPTS: Record<string, string> = {
  ...
  "8":
    'Top-down medium shot of exactly eight bright yellow bananas on a clean pastel table, arranged in two neat rows of four with empty space between every banana so toddlers can count them easily. Slow gentle push-in, soft warm daylight, colorful educational 3D-animation look, only eight bananas and no other objects. Audio: a warm friendly female narrator says cheerfully, "Eight bananas!" — no background music, natural gentle room ambience only. Mood: happy, calm, wholesome. No on-screen text, no captions, no people, no letters, no digits, no symbols. 8 seconds, 16:9.',
  "9":
