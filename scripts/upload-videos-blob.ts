...
// Optional file args (e.g. `shark-2.mp4`) limit the actual uploads to
// those files; every other clip is linked at its known stable Blob path.
const ONLY_FILES = process.argv.slice(2).filter((a) => !a.startsWith("--"));
...
async function main() {
  ...
  for (const f of files) {
    ...
    const linkOnly = MANIFEST_ONLY || (ONLY_FILES.length > 0 && !ONLY_FILES.includes(f));
    let url: string;
    if (linkOnly) {
    ...
    console.log(`[blob] ${linkOnly ? "linked" : "uploaded"} ${f} -> ${versioned}`);
  }

  const entries = [...urlsByWord.entries()]
